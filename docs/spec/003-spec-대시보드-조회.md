...
2.  시스템(BE)은 기본 필터 값(전체 학과, 최신 3개년)을 포함한 대시보드 페이지를 렌더링하여 반환한다.
3.  브라우저(FE)는 페이지 로드 직후, 기본 필터 값으로 `/api/dashboard/chart-data/`에 GET 요청을 보낸다.
