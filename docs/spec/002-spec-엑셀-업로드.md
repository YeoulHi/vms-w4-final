...
7.  각 행의 데이터를 `normalize_department()`, `normalize_metric_type()` 등의 함수를 통해 정규화하고, `metric_records` 테이블에 `UPSERT` (INSERT or UPDATE) 한다.
...
-   **데이터 파싱 오류**: 특정 행의 값(e.g., 숫자여야 할 `value`에 문자열이 포함)이 잘못된 경우, 해당 행만 실패로 기록하고 다음 행 처리를 계속 진행한다. 실패 내역은 서버 콘솔 로그에만 기록한다.
...
-   개별 행의 파싱 실패는 다른 행의 처리에 영향을 주지 않는다 (Partial Commit 허용).
...
BE -> BE: 컬럼 유효성 검증
loop 각 행(row)에 대하여
    BE -> BE: normalize_department(), normalize_metric_type()
